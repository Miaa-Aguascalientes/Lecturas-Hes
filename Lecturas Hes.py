...
# 2. ESTILO CSS (Fondo negro, bordes cian e indicadores compactos)
st.markdown("""
    <style>
        ...
        .metric-box { 
            ...
            border-right: 1px solid #333; flex: 1; padding: 5px 15px;
        }
...
st.markdown(f"""
    <div class="metric-row">
        <div class="metric-box"><img src="{ICON_METER}" class="metric-icon"><div class="metric-content"><p class="metric-label">N° de medidores</p><p class="metric-value">{len(df_mapa):,}</p></div></div>
        <div class="metric-box"><img src="{ICON_ACCUMULATED}" class="metric-icon"><div class="metric-content"><p class="metric-label">Consumo acumulado m3</p><p class="metric-value">{df_hes['Consumo_diario'].sum():,.2f}</p></div></div>
        <div class="metric-box"><img src="{ICON_AVG}" class="metric-icon"><div class="metric-content"><p class="metric-label">Prom. de Consumo diario m3</p><p class="metric-value">{df_hes['Consumo_diario'].mean():,.2f}</p></div></div>
        <div class="metric-box"><img src="{ICON_LIST}" class="metric-icon"><div class="metric-content"><p class="metric-label">Lecturas</p><p class="metric-value">{len(df_hes):,}</p></div></div>
    </div>
""", unsafe_allow_html=True)
...
with col_map:
    ...
    # CAPA DE MEDIDORES CON POPUP COMPLETO
    ...
    for _, r in df_mapa.iterrows():
        if pd.notnull(r['Latitud']):
            ...
            folium.CircleMarker(
                ...
                popup=folium.Popup(pop_html, max_width=350) # Popup restaurado
            ).add_to(m_group)
