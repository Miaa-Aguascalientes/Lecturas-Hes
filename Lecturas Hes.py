...
# FUNCIÓN PARA GENERAR GRÁFICO BASE64 PARA EL TOOLTIP
def generar_grafico_base64(df_medidor):
    ...
    fig.patch.set_facecolor('#f8f9fa') # Fondo claro para el tooltip
    ax.set_facecolor('#f8f9fa')
    
    ax.plot(df_plot['Fecha'], df_plot['Consumo_diario'], color='#007bff', marker='o', markersize=3, linewidth=2)
...
with st.sidebar:
    ...
    with st.expander("📅 RANGO DE FECHAS", expanded=True):
        opcion_rango = st.selectbox("Rango predefinido", ["Este mes", "Última semana", "Mes pasado", "Últimos 6 meses", "Este año", "Año pasado", "Personalizado"], index=0)
        ...
        with st.expander("🔍 FILTROS DE BÚSQUEDA", expanded=False):
            ...
            for col_real, nombre_amigable in mapeo_nombres.items():
                if col_real in df_hes.columns:
                    ...
                    opciones = sorted([str(int(float(x))) if str(x).replace('.0', '').isdigit() else str(x) for x in opciones_raw if pd.notnull(x) and str(x).strip() not in ['0', '0.0', '']])
        ...
        with st.expander("🏆 RANKING TOP 10", expanded=True):
            if not df_hes.empty:
                ...
                for _, row in ranking_data.iterrows():
                    try: medidor_limpio = str(int(float(row['Medidor'])))
                    except: medidor_limpio = str(row['Medidor'])
...
lat_centro, lon_centro, zoom_inicial = (df_valid_coords['Latitud'].mean(), df_valid_coords['Longitud'].mean(), 14) if not df_valid_coords.empty and (filtros_activos.get("Colonia") or filtros_activos.get("Sector")) else (21.8853, -102.2916, 12)
...
with col_map:
    ...
    if not df_sec.empty:
        for _, row in df_sec.iterrows():
            folium.GeoJson(json.loads(row['geojson_data']), style_function=lambda x: {'fillColor': '#00d4ff', 'color': '#00d4ff', 'weight': 1, 'fillOpacity': 0.1}, highlight_function=lambda x: {'fillColor': '#ffff00', 'color': '#ffff00', 'weight': 3, 'fillOpacity': 0.4}, tooltip=folium.Tooltip(f"Sector: {row['sector']}", sticky=True)).add_to(fg_sectores)

    # LOOP DE MEDIDORES CON GRÁFICO EN TOOLTIP
    for _, r in df_mapa.iterrows():
        if pd.notnull(r['Latitud']) and pd.notnull(r['Longitud']):
            color_hex, etiqueta = get_color_logic(r.get('Nivel'), r.get('Consumo_diario', 0))
            
            # FILTRAR HISTÓRICO PARA EL GRÁFICO DEL TOOLTIP
            df_hist_medidor = df_hes[df_hes['Medidor'] == r['Medidor']]
            img_base64 = generar_grafico_base64(df_hist_medidor)
            
            grafico_html = f"<div style='margin-top:10px;'><img src='{img_base64}' style='width:100%; border-radius:5px;'></div>" if img_base64 else ""

            tooltip_html = f"""
            <div style='font-family: Arial, sans-serif; font-size: 12px; color: #333; line-height: 1.4; padding: 10px; width: 320px;'>
                ...
                <b>Tarifa:</b> {r.get('Nivel', 'N/A')} | <b>Giro:</b> {r.get('Giro', 'N/A')}<br>
                <b>Sector:</b> {r.get('Sector', 'N/A')}<br>
                <b>Consumo Periodo:</b> {r.get('Consumo_diario', 0):,.2f} (m³)<br>
                <div style='text-align: center; margin-top:5px; padding: 3px; background-color: {color_hex}22; border-radius: 2px; border: 1px solid {color_hex};'>
                    <b style='color: {color_hex};'>{etiqueta}</b>
                </div>
                {grafico_html}
                <div style='font-size: 10px; color: #777; margin-top: 5px; text-align: right;'>Histórico del periodo seleccionado</div>
            </div>
            """
...
with col_der:
    ...
    if not df_hes.empty: st.dataframe(df_hes[['Fecha', 'Lectura', 'Consumo_diario']].tail(15).sort_values(by='Fecha', ascending=False), hide_index=True, use_container_width=True)
    else: st.info("No hay lecturas.")
