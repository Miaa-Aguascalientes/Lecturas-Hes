...
# ESTILO CSS (Basado en tu respaldo para compactar y dar formato)
st.markdown("""
    <style>
        ...
        /* Estilo para indicadores con iconos */
        .metric-container {
        ...
        .metric-icon { width: 45px; height: 45px; margin-right: 15px; }
...
# Iconos para indicadores según tu imagen
...
# --- INDICADORES CON ICONOS (UBICADOS ARRIBA) ---
cols_m = st.columns(4)
metrics = [
    (ICON_METER, "N° de medidores", f"{len(df_mapa):,}"),
    (ICON_ACCUMULATED, "Consumo acumulado m3", f"{df_hes['Consumo_diario'].sum():,.2f}"),
...
with col_map:
    ...
    # Capas de mapa (Negra por defecto)
    ...
    # Capa de Sectores
    if not df_sec.empty:
        sect_group = folium.FeatureGroup(name="Sectores Hidrométricos", show=True)
    ...
    # Capa de Medidores
    med_group = folium.FeatureGroup(name="Medidores", show=True)
    for _, r in df_mapa.iterrows():
        if pd.notnull(r['Latitud']) and pd.notnull(r['Longitud']):
            color_hex, etiqueta = get_color_logic(r.get('Nivel'), r.get('Consumo_diario', 0))
            pop_html = f"""<div style='font-family: Arial; font-size: 12px; width: 250px;'>
                <b>Serie:</b> {r['Medidor']}<br><b>Consumo:</b> {r.get('Consumo_diario', 0):,.2f} m3</div>"""
...
with col_der:
    ...
    if medidor_clicado:
        ...
        if not df_hes.empty:
            st.dataframe(df_hes[['Medidor', 'Fecha', 'Lectura', 'Consumo_diario']].tail(15).sort_values(by='Fecha', ascending=False), hide_index=True)
